...
data Steps pre a post hasPre hasPost where
    One :: Step pre a post hasPre hasPost -> Steps pre a post hasPre hasPost
    More :: (Compat hasPost1 hasPre2) => Step pre a post hasPre1 hasPost1 -> Steps pre a post hasPre2 hasPost2 -> Steps pre a post hasPre1 hasPost2

instance (Show a, Show pre, Show post) => Show (Steps pre a post hasPre hasPost) where
   show (One p)  = show p <> "\n"
   show (More a b) = show a <> show b

step :: IMaybe hasPre pre -> a -> IMaybe hasPost post -> Steps pre a post hasPre hasPost
step hasPre x hasPost = One $ Step hasPre x hasPost

infixr 5 >++>
(>++>)
    :: (Compat hasPost1 hasPre2)
    => Steps pre a post hasPre1 hasPost1
    -> Steps pre a post hasPre2 hasPost2
    -> Steps pre a post hasPre1 hasPost2
One x >++> ys = More x ys
More x xs >++> ys = More x $ xs >++> ys

stepsOf :: Steps pre a post hasPre hasPost -> (Maybe pre, [(a, Wedge post pre)])
stepsOf xs = case go xs of (hasPre, ys) -> (fromIMaybe hasPre, ys)
  where
    go :: Steps pre a post hasPre hasPost -> (IMaybe hasPre pre, [(a, Wedge post pre)])
